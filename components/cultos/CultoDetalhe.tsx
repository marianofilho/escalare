    ...
    >
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-sm font-medium text-zinc-800 truncate">
            ...
            {isMe && <span className="ml-1.5 text-xs text-violet-500 font-normal">(você)</span>}
          </p>
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="text-xs text-zinc-400">{inscricao.instrumento}</span>
            {inscricao.fazBacking && (
              <span className="text-xs bg-emerald-100 text-emerald-600 px-1.5 rounded-full">BV</span>
            )}
            {inscricao.ausente && (
              <span className="text-xs bg-red-100 text-red-500 px-1.5 rounded-full">Ausente</span>
            )}
          </div>
        </div>
      </div>
      ...
      {isAdmin && (
        <button
          ...
          disabled={toggling}
          title={inscricao.ausente ? "Marcar como presente" : "Marcar como ausente"}
...
export default function CultoDetalhe({ culto, repertorio, membroId, isAdmin }: CultoDetalheProps) {
  const router = useRouter()

  // estado local para mutações otimistas
  ...
  async function handleInscrever() {
    ...
    setInscricoes((prev) => [
      ...prev,
      {
        id: data.id,
        membroId,
        membroNome: data.membroNome ?? "Você",
        membroFoto: null,
        instrumento,
        fazBacking,
        ausente: false,
  ...
  async function handleCancelar() {
    if (!confirm("Cancelar inscrição neste culto?")) return
    setCancelando(true)
  ...
  async function handleAusenciaToggle(mId: string, ausente: boolean) {
    ...
    if (res.ok) {
      setInscricoes((prev) =>
        prev.map((i) => (i.membroId === mId ? { ...i, ausente } : i))
      )
    }
  }

  const podeInscrever =
    culto.status === "ABERTO" && culto.inscricoesAbertas && !jaInscrito

  // agrupa inscritos por instrumento para exibição organizada
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-zinc-400">
        <Link href="/cultos" className="hover:text-violet-600 transition-colors">
          Cultos
        </Link>
        <span>/</span>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-500 capitalize">
              ...
              {culto.dataHoraFim && (
                <span className="text-zinc-400"> até {formatarHora(culto.dataHoraFim)}</span>
              )}
            </p>
          </div>
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-zinc-100 flex-wrap">
          ...
          {culto.prazoCancelamentoHoras > 0 && (
            <p className="text-xs text-zinc-400 ml-auto">
              Cancelamento até {culto.prazoCancelamentoHoras}h antes
            </p>
          )}
        </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-zinc-800">Repertório</h2>
        </div>
        ...
        {repertorio ? (
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm text-zinc-700">
                {repertorio.totalMusicas} música{repertorio.totalMusicas !== 1 ? "s" : ""}
                <span className="text-zinc-400"> · por {repertorio.cantorNome}</span>
              </p>
            </div>
            <Link
              href={`/repertorio/${culto.id}`}
          ...
          <div className="flex items-center justify-between gap-4">
            <p className="text-sm text-zinc-400">Nenhum repertório cadastrado ainda.</p>
            <Link
              href={`/repertorio/${culto.id}`}
      ...
      {culto.status === "ABERTO" && culto.inscricoesAbertas && (
        <div className="bg-white border border-zinc-200 rounded-2xl p-5 space-y-3">
          <h2 className="text-sm font-semibold text-zinc-800">Minha participação</h2>
          ...
          {jaInscrito ? (
            ...
            <p className="text-sm text-zinc-400">Inscrições encerradas.</p>
          )}
        </div>
      )}
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-zinc-800">
            Inscritos{" "}
            <span className="font-normal text-zinc-400">({inscricoes.length})</span>
          </h2>
        </div>
